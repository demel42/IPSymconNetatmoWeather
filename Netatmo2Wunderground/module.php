...
class Netatmo2Wunderground extends IPSModule
{
	...
	private $wifi_status2text = array(
			0 => "schwach",
			1 => "mittel",
			2 => "gut",
			3 => "hoch",
		);
	private $signal_status2text = array(
			0 => "minimal",
			1 => "schwach",
			2 => "mittel",
			3 => "hoch",
	...
	private $battery_status2text = array(
			0 => "leer",
			1 => "schwach",
			2 => "mittel",
			3 => "hoch",
	...
	public function Transfer()
	{
		...
		if ($wifi_status <= 56)
			...
			$wifi_status = 2; // "good";
		else if ($wifi_status <= 86 )
			$wifi_status = 1; // "average";
		else 
			$wifi_status = 0; // "bad";
		...
		foreach (array("NAModule4", "NAModule1", "NAModule3", "NAModule2") as $types) {
			foreach ($modules as $module) {
				...
				switch($module["type"]) {
					case "NAModule1":
						...
						$battery_map = array(4000,4500,5000,5500,6000);
						break;
					case "NAModule2":
						// wind gauge
						...
						$battery_map = array(4360,4770,5180,5590,6000);
						break;
					case "NAModule3":
						// rain gauge
					...
					case "NAModule4":
						...
						$battery_map = array(4560,4920,5280,5640,6000);
						break;
				}
				...
				if ($battery_vp < $battery_map[0])
					$battery_status = 0; // "very low";
				else if ($battery_vp < $battery_map[1])
					$battery_status = 1; // "low";
				else if ($battery_vp < $battery_map[2])
					$battery_status = 2; // "medium";
				else if ($battery_vp < $battery_map[3])
					$battery_status = 3; // "high";
				else if ($battery_vp < $battery_map[4])
					$battery_status = 4; // "full";
				else
					$battery_status = 5; // "max";
				if ($battery_status < 2)
				...
				switch($module["type"]) {
					...
					case "NAModule4":
						if ($module_name == $indoor1_module_name)
							$pfx = "IN1";
						else if ($module_name == $indoor2_module_name)
							$pfx = "IN2";
						else if ($module_name == $indoor3_module_name)
							$pfx = "IN3";
